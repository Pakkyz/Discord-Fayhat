...
@client.command()
async def help(ctx):
    embed = discord.Embed(title="Command bot Fayhat",description="All available bot commands",color=0xE041A0)
...
async def on_message(message):
    ...
    if message.content == "Logout":
        ...
        await client.logout()
    elif message.content == "kuy":
        print(message.channel)
        await message.channel.send('kuy rai a i sas')
    elif message.content == "test":
        print(message.channel)
        await message.channel.send('test tam kuy rai')
...
async def play(ctx, url):
    ...
    if voice_client == None:
        ...
        #embed.insert_field_at({0}, URL, value='Now playing', inline=True)
...
async def stop(ctx):
    ...
    if voice_client.channel != ctx.author.voice.channel:
        await ctx.channel.send("The bot is currentiy connected to {0}".format(voice_client.channel))
        return
